...
_DESCRIPTORLESS_FIXTURE_PATH = os.path.join(
    scripts_root(), "shared-plugin-scripts", "mpremote", "list_serial_ports_descriptorless_cases.json"
)


def _load_shared_core_cases():
    # A missing fixture must fail loud, never skip: a skip would silently drop the
    # anti-drift contract this test exists for, and would mask either a stale nested
    # <ext>/third_party left by a pre-bump `npm run package`, or a checkout whose
    # submodule pin predates the fixture. Let open() raise FileNotFoundError itself
    # (pre-checking with os.path.exists would mis-report a PermissionError the same
    # way) and name the resolved path so the cause is diagnosable in one read.
    try:
        ...
        raise FileNotFoundError(
            ...
            "(stale third_party checkout, or submodule pinned before the fixture landed?)"
        ) from exc
