...
const REPLACE_HW_RE = /\b(replace|swap)\b[^.]*\b(sensor|module|hardware|component|device|part)\b/i;
...
const DEPLOY_RE = /\b(deploy|upload|flash)\b[^.]*\b(fail|error|not work|broke|crash)\b|\b(fail|error|crash)\b[^.]*\b(deploy|upload)\b/i;
