...
///
/// `include!`d by BOTH build scripts, `cli/build.rs` and `app/build.rs`:
/// the GUI exe is the binary a user actually runs, so a guard on the CLI
/// alone would have left the shipped one unprotected.
///
