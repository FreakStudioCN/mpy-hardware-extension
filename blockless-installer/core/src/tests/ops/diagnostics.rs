...
/// bundle: with no loadable manifest, logs and the journal still ship, and
/// the load error travels inside `facts.json` instead of being the reason
/// nothing was written.
#[test]
fn diagnostics_bundle_without_a_manifest_still_carries_logs_and_state() {
    let dir = temp_dir("diagnostics-no-manifest");
    let manifest = test_manifest();
    let vsix = write_vsix(&dir, b"vsix contents");
    let ctx = make_ctx(&dir, &manifest, &vsix);
    std::fs::create_dir_all(&ctx.paths.logs).unwrap();
    ...
    write_diagnostics_bundle(
        ...
        ctx.os,
        ctx.arch,
        Err("could not read installer.manifest.json; searched: /nowhere"),
        &zip_path,
    )
    .unwrap();
    ...
    assert!(
        ...
        "no manifest was loaded, so none may be claimed: {names:?}"
    );
    ...
    assert_eq!(
        facts["manifestError"],
        "could not read installer.manifest.json; searched: /nowhere"
    );
    assert!(facts.get("installerVersion").is_none());
}
