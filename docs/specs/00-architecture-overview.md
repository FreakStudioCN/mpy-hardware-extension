...
**护城河**：① CEO 是 MicroPython 核心贡献者 ② 已有 173 个 uPyPi 真包（治 LLM 幻觉）③ board manifest 白名单（治 5V 接 3.3V 烧板）④ skill 渐进披露库的迭代节奏（治代码风格漂移 + 不断累积真机错误样本）。
...
| `mpyhw-api` | **LLM 薄代理 + content 仓库 + 配额 + 防滥用**：透传 Anthropic Messages API（SSE 流式）、托管 skill / board profile / tool registry 静态内容、代理 uPyPi、接收 telemetry | Python (FastAPI) | 内部部署（Fly.io / Railway） |
...
6. 流到 tool_use block → ext 立即本地 dispatch:
   ...
   ├─ api-proxy   : search_upypi / get_package_metadata (调 /v1/upypi/*)
   ├─ nested-LLM  : generate_code (开新 SSE 子流调 /v1/llm/messages, 拿生成代码)
   ├─ shim        : scan_device / flash_and_run / read_serial_until / write_main_py
...
| 3 | mpremote / uPyPi / 串口实现 | 官方 `mpremote` CLI（subprocess）+ `httpx` 调 uPyPi REST + `pyserial` 标准库 |
...
| 14 | **项目模板库** | 内置 3 个模板（温度阈值 LED / 按钮蜂鸣器 / OLED 显示）打包扩展 `resources/templates/`；首屏卡片选择器选模板 → 自动填 intent + 推荐 board / pin |
...
| 术语 | 含义 |
|---|---|
| **manifest** | `HardwareManifest`，**TS zod schema** 定义，包含 board、传感器、pin 分配、阈值、逻辑方向。从 user intent + tool calls 派生。**Schema 现在住在客户端**。 |
...
| **tool** | agent 可调用的能力，按执行位置分四类：**local TS**（query_board_profile / propose_manifest / audit_code / load_skill）、**api-proxy**（search_upypi / get_package_metadata）、**shim**（scan_device / flash_and_run / write_main_py / read_serial_until）、**ui-prompt**（ask_user） |
| **intent gate** | 服务端 middleware 在透传 Anthropic Messages API 第一轮时强制注入到 system 首段的判断指令；client 不可见 |
| **canonical tool registry** | mpyhw-api 维护的权威 tool 列表（11 个内置）；client 发的 tools 字段必须全部命中 |
...
| 版本 | 内容 |
|---|---|
| **v0.2.0**（本 MVP） | 两仓全发，VS Code Marketplace 发布；demo: **ESP32-S3 + AHT20（或 DS18B20）+ LED**（实测在 uPyPi 上存在）；3 块板内置；inline intent gate（middleware）；skill 渐进披露完整实现；**SSE 流式从 day 1**；隐形 git auto-commit；接线表输出；3 个内置项目模板；`mpremote mip install` 包安装到设备 |
| v0.3.0 | GitHub OAuth + 注册用户额度；Linux 支持；Architect+Editor 两阶段生成；评估切 belay-py；评估 bundle 嵌入式 Python runtime；**BYOK 直连 Anthropic 跳过 api**；SSE 断流恢复（`Last-Event-Id`）；**Git Timeline UI + 一键回滚**；**测试代码生成（test_main.py）**；`GET /v1/upypi/index` 批量包索引；PDF→驱动转换；Arduino→MicroPython 转换；GitHub Search API fallback；硬件型号推荐；报错周期分析；submodule CI/CD 同步 skill；网页项目库 SEO 入口 |
