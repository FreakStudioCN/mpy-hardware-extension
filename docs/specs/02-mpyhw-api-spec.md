...
This spec preserves some historical V3 target text. For current code, treat the
following as authoritative:

- `POST /v1/llm/messages` is protected by GitHub-session JWT auth and
  token-metered credits. A client cannot bypass credits by calling the LLM route
  directly.
- The backend uses an LLM provider adapter. The default is DeepSeek through an
  OpenAI-compatible chat-completions stream; it is translated into
  Anthropic-shaped SSE events for the extension.
- The provider is selected by `MPYHW_LLM_PROVIDER`; unsupported providers fail
  closed with `llm_provider_not_supported`.
- `/v1/quota` has been removed. Use `GET /v1/credits` for balance display.
- `<not_hardware>` intent-gate middleware and Anthropic prompt-cache
  pass-through are legacy design assumptions, not current implementation
  guarantees.
...
| `GET`  | `/v1/credits` | Current authenticated credit balance |
...
mpyhw-api/
  ...
  routes/
    ...
    routes_credits.py                 # GET /v1/credits                     ~30 lines
...
- `GET /v1/credits` -> returns the authenticated user's current balance
