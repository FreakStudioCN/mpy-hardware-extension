...
| Marketplace publisher | `mpyhw` |
| Extension name | `mpy-hardware-extension` |
| Marketplace ID | `mpyhw.mpy-hardware-extension` |
| 展示名 | `Blockless` |
| 当前版本 | `0.3.3` |
...
- 一个 Visual Studio Marketplace publisher，当前项目应使用 `mpyhw`。
...
如果 Marketplace 上还没有 `mpyhw` 这个 publisher，需要创建它；如果已经存在，
...
进入 `mpyhw` publisher，把上一步拿到的 managed identity resource ID 加为成员，
角色给 `Contributor`。

这一步完成后，pipeline 里的 `vsce publish --azure-credential` 才有权限把
`mpyhw.mpy-hardware-extension` 发布到 Marketplace。
...
```text
mpyhw.mpy-hardware-extension
```

Marketplace 页面通常是：

```text
https://marketplace.visualstudio.com/items?itemName=mpyhw.mpy-hardware-extension
```
...
4. 创建 namespace，必须和 `package.json` 的 `publisher` 一致：
   ```powershell
   npx ovsx create-namespace mpyhw -p <OVSX_TOKEN>
   ```
5. 发布已经打好的 VSIX：
   ```powershell
...
- Managed identity 的 federated credential 里的 issuer / subject 是否和 service
  connection 完全匹配。
- Managed identity resource ID 是否已经被加到 `mpyhw` publisher 成员里。
...
- [ ] `package.json` 的 `publisher` 是 `mpyhw`。
...
- [ ] Managed identity 已加入 `mpyhw` publisher，角色是 `Contributor`。
